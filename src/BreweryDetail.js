...
export default function BreweryDetail() {
  const { brewery, setBrewery } = useContext(AppContext);
  ...
  useEffect(() => {
    if(sessionStorage.getItem('brewery') === null) {
      sessionStorage.setItem('brewery', JSON.stringify(brewery));
    } else {
      setBrewery(JSON.parse(sessionStorage.getItem('brewery')));
    }
  },[])
